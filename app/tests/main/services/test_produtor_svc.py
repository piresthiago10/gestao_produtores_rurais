...
async def test_get_by_email(get_db, producers_in_db):
    ...
    # erro ao buscar
    with mock.patch(
        "app.database.crud_produtor.CRUD_Produtor.get_by_email"
...
async def test_get_by_cpf_cnpj(get_db, producers_in_db):
    ...
    # erro ao buscar
    with mock.patch(
        "app.database.crud_produtor.CRUD_Produtor.get_by_cpf_cnpj"
...
@pytest.mark.asyncio
async def test_handle_farm_in_producer(get_db, producers_in_db, farms_in_db):
    """Testa a associação de um produtor com uma fazenda."""
    crud = CRUD_Produtor(get_db)
    produtor_service = ProdutorService(Produtor, crud)
    result = await produtor_service.handle_farm_in_producer(Fazenda, 1, 1)
    assert result == RESULT_PRODUCER_WITH_FARM.copy()

    # id produtor inexistente
    with pytest.raises(ValueError) as excinfo:
        await produtor_service.handle_farm_in_producer(Fazenda, 1000, 1)
    excinfo.match("Produtor com ID 1000 não encontrado.")

    # id fazenda inexistente
    with pytest.raises(ValueError) as excinfo:
        await produtor_service.handle_farm_in_producer(Fazenda, 1, 1000)
    excinfo.match("Fazenda com ID 1000 não encontrada.")

    # removendo produtor da fazenda
    result = await produtor_service.handle_farm_in_producer(Fazenda, 1, 1, is_add=False)
    assert result == RESULT_PRODUCER_WITHOUT_FARM.copy()
